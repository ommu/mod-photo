...
class PhototagController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = AlbumPhotoTag::model()->findByPk(1,array(
		...
		$this->pageTitle = Yii::t('phrase', 'Album Photo Tags');
		$this->pageDescription = $setting->meta_description;
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/photo_tag/front_index',array(
	...
	public function actionView($id) 
	{
		...
		$this->pageTitle = Yii::t('phrase', 'View Album Photo Tags');
		$this->pageDescription = '';
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/photo_tag/front_view',array(
			'model'=>$model,
		));
		/*
		$this->render('/photo_tag/admin_view',array(
			'model'=>$model,
		));
		*/
	}	
	...
	public function actionManage() 
	{
		...
		$this->pageDescription = '';
		$this->pageMeta = '';
		$this->render('/photo_tag/admin_manage',array(
			'model'=>$model,
			'columns' => $columns,
		));
	}	
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['AlbumPhotoTag'])) {
			$model->attributes=$_POST['AlbumPhotoTag'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'AlbumPhotoTag success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'AlbumPhotoTag success created.'));
		...
		$this->pageTitle = Yii::t('phrase', 'Create Album Photo Tags');
		$this->pageDescription = '';
		$this->pageMeta = '';
		$this->render('/photo_tag/admin_add',array(
			'model'=>$model,
		));
	}
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['AlbumPhotoTag'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-album-photo-tag',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'AlbumPhotoTag success updated.').'</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'AlbumPhotoTag success updated.'));
					//$this->redirect(array('view','id'=>$model->id));
					$this->redirect(array('manage'));
				}
			}
		...
		$this->pageTitle = Yii::t('phrase', 'Update Album Photo Tags');
		$this->pageDescription = '';
		$this->pageMeta = '';
		$this->render('/photo_tag/admin_edit',array(
			'model'=>$model,
		));
	}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				AlbumPhotoTag::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				AlbumPhotoTag::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			$this->pageDescription = '';
			$this->pageMeta = '';
			$this->render('/photo_tag/admin_delete');
		}
	}
	...
	public function actionPublish($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			$this->pageDescription = '';
			$this->pageMeta = '';
			$this->render('/photo_tag/admin_publish',array(
				'title'=>$title,
				'model'=>$model,
			));
		}
	...
	public function actionHeadline($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			$this->pageDescription = '';
			$this->pageMeta = '';
			$this->render('/photo_tag/admin_headline');
		}
	}
